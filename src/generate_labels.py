...
def generate_labels():
    img_center_coords = read_coordinates(filename='coord_list')
...
def convert_to_tlbr(center_coords: List[Tuple[str, str]]) -> np.ndarray:
    half_width_webm = 0.002641 / 1832 * cfg.IMAGE_SIZE_w
    half_height_webm = 0.0010985 / 956 * cfg.IMAGE_SIZE_h
    ...
    x_min, y_min = lonlat_to_web.transform(center_lonlat[:, 1] - half_height_webm, center_lonlat[:, 0] + half_width_webm)
    x_max, y_max = lonlat_to_web.transform(center_lonlat[:, 1] + half_height_webm, center_lonlat[:, 0] - half_width_webm)
...
def convert_geometry_to_image_points(geom, tlbr):
    ...
    y_pixels = ((cfg.IMAGE_SIZE_h * (tlbr[3] - y) / (tlbr[3] - tlbr)).astype(np.int32))
