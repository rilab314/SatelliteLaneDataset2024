    ...
    for geom in geometries:
        if isinstance(geom, Polygon):
            ...
            for interior in geom.interiors:
                interior_coords = [coords_to_pixels(x, y, x_min, y_max, x_max, y_min, width, height) for x, y in np.array(interior.coords)]
                cv2.polylines(img, [np.array(interior_coords, dtype=np.int32)], True, (255, 0, 255), 1)
        elif isinstance(geom, LineString):
            line_pixels = [coords_to_pixels(x, y, x_min, y_max, x_max, y_min, width, height) for x, y in np.array(geom.coords)]
            cv2.polylines(img, [np.array(line_pixels, dtype=np.int32)], False, (0, 255, 255), 1)
...
image_list = glob("/media/falcon/50fe2d19-4535-4db4-85fb-6970f063a4a11/Ongoing/2024_SATELLITE/dataset/국토정보플랫폼/gookto_image/copy_00001_2_surface/nobox/*.png")
image_list.sort()
