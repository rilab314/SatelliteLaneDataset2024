        ...
        for metric in self.metrics:
            ...
            if metric == 'proposal':
                ...
                if self.classwise:  # Compute per-category AP
                    # Create a mapping from self.cat_ids to the index in coco_eval.params.catIds
                    catid_to_idx = {cat_id: idx for idx, cat_id in enumerate(coco_eval.params.catIds)}
                    ...
                    for cat_id in self.cat_ids:  # Use self.cat_ids order
                        if cat_id not in catid_to_idx:
                            continue
                        idx = catid_to_idx[cat_id]  # Find the corresponding index
                        t = []
                        # area range index 0: all area ranges
                        # max dets index -1: typically 100 per image
                        nm = self._coco_api.loadCats(int(cat_id))[0]
                        ...
                        # indexes of area of small, medium, and large
                        for area in [1, 2, 3]:
